...
class Special_ParallelSampler():
    ...
    def get_epoch(self):
        for _ in range(self.num_episodes):
            ...
            support, query, mask = self.done_queue.get()

            # convert to torch.tensor
            support = utils.to_tensor(support, self.args.cuda, ['raw', 'il_text'])
            query = utils.to_tensor(query, self.args.cuda, ['raw', 'il_text'])
    ...
    def worker(self, done_queue):
        ...
        while True:
            ...
            support = utils.select_subset(self.data, {}, ['text', 'il_text', 'text_len', 'label', 'raw'],
                                     support_idx, max_support_len)
            query = utils.select_subset(self.data, {}, ['text', 'il_text', 'text_len', 'label', 'raw'],
                                   query_idx, max_query_len)
            
            # Apply the filter
            mask = filter.analyze_df(query, self.args) # self.vocab)
            # FLAG = filter.analyze_struct(query, self.args) # self.vocab)
            # if FLAG: continue
            ...
            done_queue.put((support, query, mask))
